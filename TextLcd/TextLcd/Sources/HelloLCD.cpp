...
inline uint8_t reverse_data(uint8_t data, uint32_t length = 1)
{
...
unsigned char LCD_rCommand(void){
    ...
    temp = reverse_data(LCD_DATA_IN);      // 명령 읽기
    _delay_us(1);
...
void LCD_wCommand(char cmd){
    ...
    LCD_DATA = reverse_data(cmd);          // 명령 출력
    _delay_us(1);
...
void LCD_wData(char dat){
    ...
    LCD_DATA = reverse_data(dat);       // 데이터 출력
    _delay_us(1);
...
int main(void){
    ...
    LCD_wBCommand(0x01 | 0x00);  // DDRAM Address = 0 설정
    LCD_wString("TEXT LCD");     // 텍스트 LCD 문자열 출력
 
 
    LCD_wBCommand(0x01 | 0x02);  // DDRAM Address = 0x40 설정
    LCD_wString("HELLO WORLD!"); // WESNET 문자열 출력
